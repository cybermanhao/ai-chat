...
export function extractXmlContent(xml: string) {
...
export function extractPluginContent(xml: string) {
  // 兼容旧实现，实际应由 engine/utils/xml.ts 提供
  const matches = [...xml.matchAll(/<plugin name="([^"]+)">([\s\S]*?)<\/plugin>/g)];
  return matches.map(m => ({ pluginId: m[1], content: m[2] }));
}

export function processPluginContent(content: string, plugin: any) {
  // 兼容旧实现，实际应由 engine/utils/xml.ts 提供
