...
export class MCPClient {
  ...
  async connect() {
    console.log(`[MCPClient] 开始连接服务器... URL: ${this.url}, 连接类型: ${this.connectionType}`);
    
    // 如果已经连接，先断开
    if (this.connected) {
      console.log('[MCPClient] 检测到已有连接，先断开...');
      await this.disconnect();
    }
    
    // 创建新的MCP客户端实例
    this.mcp = new Client({ name: this.clientName, version: this.clientVersion });
    console.log(`[MCPClient] 已创建新的 MCP 客户端, 名称: ${this.clientName}, 版本: ${this.clientVersion}`);

    try {
      switch (this.connectionType) {
        case "STDIO":
          if (typeof window !== "undefined") throw new Error("STDIO not supported in browser");
          console.log(`[MCPClient] 使用 STDIO 传输, 命令: ${this.serverCommand}, 参数:`, this.serverArgs);
        ...
        case "SSE":
          ...
          console.log(`[MCPClient] 使用 SSE 传输, URL: ${this.url}`);
        ...
        case "STREAMABLE_HTTP":
          ...
          console.log(`[MCPClient] 使用 StreamableHTTP 传输, URL: ${this.url}`);
      ...
      console.log('[MCPClient] 开始建立连接...');
      await this.mcp.connect(this.transport);
      this.connected = true;
      console.log('[MCPClient] 连接成功!');
    } catch (error) {
  ...
  async listTools(): Promise<{ data: Tool[]; error?: string }> {
    try {
      console.log('[MCPClient] 检查连接状态:', this.connected ? '已连接' : '未连接');
      if (!this.connected) {
        console.log('[MCPClient] 尚未连接，尝试建立连接...');
        await this.connect();
      }
      console.log('[MCPClient] 开始获取工具列表...');
      const toolsResponse = await this.mcp.listTools();
      console.log('[MCPClient] 原始工具列表响应:', toolsResponse);
      ...
      console.log('[MCPClient] 处理后的工具列表:', tools);
      return { data: tools };
    } catch (e: any) {
      console.error('[MCPClient] 获取工具列表失败:', e);
  ...
  async callTool(name: string, args: Record<string, any>): Promise<{ data: any; error?: string }> {
    try {
      console.log(`[MCPClient] 调用工具 ${name}, 参数:`, args);
      if (!this.connected) {
        console.log('[MCPClient] 尚未连接，尝试建立连接...');
        await this.connect();
      }
      console.log('[MCPClient] 开始调用工具...');
      const result = await this.mcp.callTool({ name, arguments: args });
      console.log(`[MCPClient] 工具 ${name} 调用成功:`, result);
      return { data: result };
    } catch (e: any) {
      console.error(`[MCPClient] 工具 ${name} 调用失败:`, e);
  ...
  async disconnect() {
    console.log('[MCPClient] 开始断开连接...');
    
    if (!this.connected && !this.mcp) {
      console.log('[MCPClient] 已经断开连接，无需操作');
      return;
    }
    ...
    try {
      ...
      if (this.mcp) {
        ...
        console.log('[MCPClient] MCP客户端已关闭');
      }
    } catch (error) {
      console.error('[MCPClient] 关闭MCP客户端时出错:', error);
    ...
    this.transport = null;
    this.mcp = null;
    console.log('[MCPClient] 连接状态已完全重置');
    console.log('[MCPClient] 断开连接完成');
  }
}

/**
 * 通用工具调用，自动处理 loading/done/error 状态
 * @param mcp MCPClient 实例
 * @param name 工具名
 * @param args 工具参数
